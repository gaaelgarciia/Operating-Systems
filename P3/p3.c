...
void processEntry(char *tr[],tListP *L,int *terminado,int *total,char *cmd,tListM *M,tListC *C);  //procesa el comando y realiza el comando correspondiente
...
void Cmd_comand(char *tr[],tListP L,int terminado,int total,char *cmd,tListM M,tListC C){
    tPos p;
    char *trozos[MAX];
    int a = atoi(tr[1]),b;
    if(tr[1]==NULL){
        ...
        if(a >= 0 ){
            ...
            for(p=first(L);b<=a;p= nextP(p,L)){
                if(b==a){
                    ...
                    processEntry(trozos,&L,&terminado,&total,cmd,&M,&C);
                    break;
...
void Cmd_help(char *tr[],char *cmd){
    if(tr[1]==NULL){
        ...
        printf("views (or changes) the process's credentials\n");
    }else if(strcmp(tr[0],"showvar")==0) {
        printf("shows the value of an environment variable\n");
    }else if(strcmp(tr[0],"changevar")==0) {
        printf("changes the value of an environment variable\n");
    }else if(strcmp(tr[0],"subsvar")==0) {
        printf("changes one environmet variable for other\n");
    }else if(strcmp(tr[0],"showenv")==0) {
        printf("shows the process environment\n");
        ...
        printf("executes a program (with arguments) without creating a new process. It is not neccesary to\n"
               "do the priority (@pri) nor the reduced environment (VAR1 VAR2..) parts\n");
    }else if(strcmp(tr[0],"jobs")==0) {
        printf("lists background processes\n");
    }else if(strcmp(tr[0],"deljobs")==0) {
        printf("deletes background processes from the list\n");
    }else if(strcmp(tr[0],"job")==0) {
        printf("shows info on a background process. Can change it to foreground\n");
    }else if(strcmp(tr[0],"*****")==0) {
        printf("For anything that is not a shell command, the shell will asume it is an external program (in\n"
...
void Cmd_quit(char *tr[],int *terminado,tListP *L,char *command, tListM *M,tListC *C){
    if(tr[1]==NULL){
...
char* horaactual(){
    ...
    t= time(NULL);
    tm = localtime(&t);
    byteEsc=strftime(date, 50, "%Y/%m/%d %H:%M:%S", tm);
    if(byteEsc!=0)
        return date;
    else{
...
void mostrar_credencial(){
    uid_t real_uid=getuid();
    uid_t effective_uid=geteuid();

    struct passwd *real_user_info= getpwuid(real_uid);
    struct passwd *effective_user_info= getpwuid(effective_uid);

    printf("Credencial real: %d, (%s)\n",real_uid,real_user_info->pw_name);
    printf("Credencial efectiva: %d, (%s)\n",effective_uid,effective_user_info->pw_name);
}
void set_crendencial(uid_t id){
    if(setuid(id)!=0){
        perror("Imposible cambiar credencial");
    }
}
void set_login_crendencial(const char *login){
    ...
    if(user_info!=NULL) {
        ...
        fprintf(stderr,"Usuario no existente %s\n",login);
    }
}

void Cmd_uid(char *tr[]){
    if(tr[1]==NULL || !strcmp(tr[1],"-get")){
        mostrar_credencial();
    }else if(!strcmp(tr[1],"-set")){
        if(tr[2]!=NULL){
            if(!strcmp(tr[2],"-l") && tr[3]!=NULL){
                const char *login= tr[3];
                set_login_crendencial(login);
...
void Cmd_fork (char *tr[],tListC *C)
{
    pid_t pid;
...
int CambiarValor(char *var1, char *var2, char *valor, char *e[]) {
    int posVar1;
    char *aux;

    // Buscar posición de la variable a sustituir
    if ((posVar1 = BuscarVariable(var1, e)) == -1){
        return (-1);}

    if((BuscarVariable(var2, e)) != -1){
        return (-1);}
...
void print_var(char *env[],char *name){
    if (env == NULL || name == NULL) {
        return;  // Manejar punteros nulos
    }
    for(int a=0;env[a]!=NULL;a++){
        if(env[a] != NULL){
            printf("%p->%s[%d]=(%p) %s\n",(void *)&env[a],name,a,(void *)env[a],env[a]);
        }else{
            printf("%p->%s[%d]=(%p) (null)\n",(void *)&env[a],name,a,(void *)env[a]);
        }
    }
}
...
void print_show(int a, int e, char *value, char *arg3[], char *environ[]){
    ...
    printf("Con arg3 main: \t%s (%p) %p\n", arg3[a],(void *) arg3[a], (void *)&arg3[a]);
    printf("Con environ: \t%s (%p) %p\n", environ[e], (void *)environ[e], (void *)&environ[e]);
...
void Cmd_changevar(char *tr[]){
    char *var_aux =(char *)malloc(strlen(tr[2]) + strlen(tr[3]) + 2);
    ...
    if(!strcmp(tr[1], "-a")){
        if(CambiarVariable(tr[2], tr[3], arg3)==-1){
        ...
        if(putenv(var_aux)!=0){
            perror("Imppsible cambiar variable");
        }
        return;
    }else{
        printf("Uso: changevar [-a|-e|-p] var valor\n");
...
void Cmd_subsvar(char *tr[]){
    if(tr[1]==NULL || tr[2]==NULL || tr[3]==NULL || tr[4]==NULL ){
        fprintf(stderr,"Uso: subsvar [-a|-e] var valor\n");
        return;
    }
    if(!strcmp(tr[1],"-a")){
        int result=CambiarValor(tr[2],tr[3],tr[4],arg3);
        if(result==-1){
            fprintf(stderr,"Imposible sustituir variable %s por %s\n", tr[2], tr[3]);        }
    }
    else if(!strcmp(tr[1],"-e")){
        int result=CambiarValor(tr[2],tr[3],tr[4],environ);
        if(result==-1){
            fprintf(stderr,"Imposible sustituir variable %s por %s\n", tr[2], tr[3]);
        }
    }else{
...
void Cmd_showenv(char *tr[]) {
    if (tr[1] == NULL) {
        print_var(arg3, "main arg3");
    } else if (!strcmp(tr[1], "-environ")) {
        print_var(environ, "environ");
    } else if (!strcmp(tr[1], "-addr")) {
...
void Cmd_exec(char *tr[], tListC *C){
    char *arg[MAX/2], *cmd, *prio;
    int back = 0, execute = 0, i, priority = 0, posarg = 0, totalLength = 0;

    if(strcmp(tr[0], "exec") == 0)
        execute = 1;
    if (execute && tr[1] == NULL){
    ...
    if(!execute){
        ...
        if(pid == -1)
            ...
            if(back){cmd = malloc(totalLength + 1);
                strcpy(cmd,tr[0]);
                for(int j = 1; j < i; j++){
                    strcat(cmd, " ");
                    strcat(cmd, tr[j]);
                }
                char *time = horaactual();
                insertProcess(pid, time, cmd, priority, C);
                free(cmd);}
            else{
                waitpid(pid,NULL, 0);
            }
...
void Cmd_jobs(char *tr[],tListC *C){
    mostrarListaProcesos(*C);
}

void Cmd_deljobs(char *tr[],tListC *C){
    if(tr[1]==NULL){
        mostrarListaProcesos(*C);
    }
    else if(!strcmp(tr[1],"-term")){
        deleteTerminatedProcess(C);
    }
    else if(!strcmp(tr[1],"-sig")){
        deleteTerminatedSignProcess(C);
    }
    else{
        mostrarListaProcesos(*C);
    }
...
void Cmd_job(char *tr[],tListC *C){
    ...
    pid = (pid_t) strtol(tr[i],NULL,10);
    if(pid==0){
        mostrarListaProcesos(*C);
        return;
    }
    pos=buscarProceso(pid,*C);
    if(pos==NULL){
        mostrarListaProcesos(*C);
        return;
    }
    if(!fg){
        mostrarProceso(pos, C);
        return;
    }
    deleteProcess(pos, C);
}
...
void processEntry(char *tr[],tListP *L,int *terminado ,int *total,char *command,tListM *M,tListC *C){
    int n=0;
    if (tr[0] != NULL) {

        if (strcmp(tr[0], "authors") == 0) {
            Cmd_authors(tr,command);
        } else if (strcmp(tr[0], "pid") == 0) {
            Cmd_pid(tr,command);
        } else if (strcmp(tr[0], "chdir") == 0) {
            Cmd_chdir(tr,command);
        } else if (strcmp(tr[0], "date") == 0) {
            Cmd_date(tr,command);
        } else if (strcmp(tr[0], "time") == 0) {
            Cmd_time(tr,command);
        } else if (strcmp(tr[0], "hist") == 0) {
            Cmd_hist(tr,*L,command);
        } else if (strcmp(tr[0], "comand") == 0) {
            Cmd_comand(tr,*L,*terminado,*total,command,*M,*C);
...
int main(int argc,char *argv[],char *envp[]) {
    ...
    while(terminado!=0) {
        ...
        GuardaHist(cmd, tr,&L);
        processEntry(tr,&L,&terminado,&total,cmd,&M,&C);
    }
    return 0;
}
